...
import app.libre.extensions.parcelableExtra
...
import app.libre.util.DefaultTrackSelectorWithAudioQualitySupport
import app.libre.util.NowPlayingNotification
import app.libre.util.PauseableTimer
    ...
    open fun runPlayerCommand(args: Bundle) {
        when {
            ...
            args.containsKey(PlayerCommand.TOGGLE_AUDIO_ONLY_MODE.name) -> {
                val newAudioOnly = args.getBoolean(PlayerCommand.TOGGLE_AUDIO_ONLY_MODE.name)
                val switchingToVideo = isAudioOnlyPlayer && !newAudioOnly
                isAudioOnlyPlayer = newAudioOnly
                trackSelector?.updateParameters {
                ...
                if (switchingToVideo) {
                    ...
                    startTimestampSeconds = currentPos / 1000
