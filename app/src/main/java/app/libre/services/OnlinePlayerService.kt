...
open class OnlinePlayerService : AbstractPlayerService() {
    ...
    override suspend fun startPlayback() {
        ...
        fetchVideoInfoJob = scope.launch {
            ...
            val localPath = if (isAudioOnlyPlayer) app.libre.helpers.LocalAudioMatcher.getLocalPathAsync(videoId) else null
            val isOffline = !app.libre.helpers.NetworkHelper.isNetworkAvailable(this@OnlinePlayerService)

            if (localPath != null) {
                // LOCAL FILE EXISTS — play instantly with cached/local metadata!
                
                // 1. If online, fetch fresh metadata silently in background to update cache for next time
                if (!isOffline) {
                    scope.launch(Dispatchers.IO) {
                        try {
                            val s = MediaServiceRepository.instance.getStreams(videoId)
                            app.libre.db.DatabaseHolder.Database
                                .localAudioMetadataCacheDao().insert(
                                    app.libre.db.obj.LocalAudioMetadataCache(
                                        videoId = videoId,
                                        title = s.title,
                                        uploader = s.uploader,
                            ...
                            Log.w(TAG(), "Could not fetch online metadata for local track in background: ${e.message}")
                        }
                    }
                }

                // 2. Build streams immediately from cache or embedded file info
                ...
                val streamItem = playlistItem?.toStreamItem() ?: PlayingQueue.getCurrent()
                ...
                streams = Streams(
                    title = app.libre.helpers.LocalAudioMatcher.getTitleFromFile(videoId) ?: cached?.title?.takeIf { it.isNotBlank() } ?: streamItem?.title ?: "Local Audio",
                    uploader = cached?.uploader?.takeIf { it.isNotBlank() } ?: streamItem?.uploaderName ?: "Local Artist",
                    artist = app.libre.helpers.LocalAudioMatcher.getArtistFromFile(videoId),
