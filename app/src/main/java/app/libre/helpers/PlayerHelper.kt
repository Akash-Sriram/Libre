...
import app.libre.obj.VideoStats
...
object PlayerHelper {
    ...
    private fun getCache(context: Context): androidx.media3.datasource.cache.SimpleCache {
        if (simpleCache == null) {
            ...
            simpleCache = androidx.media3.datasource.cache.SimpleCache(
                cacheDir,
                androidx.media3.datasource.cache.LeastRecentlyUsedCacheEvictor(1024 * 1024 * 512), // 512MB max cache size
                databaseProvider
            )
        }
        return simpleCache!!
    ...
    fun getLoadControl(context: Context): LoadControl {
        val isMetered = app.libre.helpers.NetworkHelper.isNetworkMetered(context)
    ...
    fun getPlayPauseActionIcon(player: Player) = when {
        player.playWhenReady && player.playbackState != Player.STATE_ENDED -> R.drawable.ic_pause
