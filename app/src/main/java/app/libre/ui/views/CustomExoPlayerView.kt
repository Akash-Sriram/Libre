...
import app.libre.databinding.PlayerGestureControlsViewBinding
...
import app.libre.helpers.BrightnessHelper
...
import app.libre.obj.VideoResolution
import app.libre.services.AbstractPlayerService
import app.libre.ui.base.BaseActivity
import app.libre.ui.controllers.FullscreenGestureAnimationController
import app.libre.ui.extensions.toggleSystemBars
import app.libre.ui.interfaces.CustomPlayerCallback
import app.libre.ui.interfaces.PlayerGestureOptions
import app.libre.ui.interfaces.PlayerOptions
import app.libre.ui.listeners.PlayerGestureController
...
class CustomExoPlayerView(
            ...
            val sheet = chaptersBottomSheet ?: ChaptersBottomSheet()
                .apply {
                    arguments = Bundle().apply {
                        putLong(IntentData.duration, player.duration.div(1000))
                    }
                }
                .also {
    ...
    fun updateCurrentSubtitle(trackId: String?) {
        ...
        player.sendCustomCommand(
            AbstractPlayerService.runPlayerActionCommand, Bundle().apply {
                putString(PlayerCommand.SET_CAPTION_TRACK.name, trackId)
            }
        )
    }
    ...
    fun setPlayerResolution(resolution: Int, isSelectedByUser: Boolean = false) {
        ...
        player.sendCustomCommand(
            AbstractPlayerService.runPlayerActionCommand, Bundle().apply {
                putInt(PlayerCommand.SET_RESOLUTION.name, transformedResolution)
            }
        )
    ...
    override fun onAudioStreamClicked() {
        ...
        if (audioLanguagesAndRoleFlags.isEmpty() || (audioLanguagesAndRoleFlags.size == 1 &&
            ...
            baseBottomSheet.setSimpleItems(
                ...
                player.sendCustomCommand(
                    AbstractPlayerService.runPlayerActionCommand, Bundle().apply {
                        putInt(PlayerCommand.SET_AUDIO_ROLE_FLAGS.name, selectedAudioFormat.second)
                    }
                )
                player.sendCustomCommand(
                    AbstractPlayerService.runPlayerActionCommand, Bundle().apply {
                        putString(PlayerCommand.SET_AUDIO_LANGUAGE.name, selectedAudioFormat.first)
                    }
                )
