    ...
    override fun queueInput(inputBuffer: ByteBuffer) {
        ...
        when (encoding) {
            C.ENCODING_PCM_16BIT -> repeat(sampleCount) {
                ...
                output.putShort(
                    if (applyGain) {
                        (sample * gain.linearGain).coerceIn(-32768.0, 32767.0).toInt().toShort()
                    } else {
                        sample
                    }
                )
            ...
            C.ENCODING_PCM_24BIT -> repeat(sampleCount) {
                ...
                val processed = if (applyGain) {
                    (sample * gain.linearGain).coerceIn(-8388608.0, 8388607.0).toInt()
                } else {
                    sample
                }
            ...
            C.ENCODING_PCM_32BIT -> repeat(sampleCount) {
                ...
                output.putInt(
                    if (applyGain) {
                        (sample * gain.linearGain).coerceIn(-2147483648.0, 2147483647.0).toInt()
                    } else {
                        sample
                    }
                )
